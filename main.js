...
function registerHandlers() {
  ...
  ipcMain.handle("update-workbook", async (_event, payload = {}) => {
    const { records = [], sheetName = SHEET_NAME, containers = [], folders = [], albumFolders = [] } = payload;
  ...
  ipcMain.handle("export-xlsx", async (_event, payload = {}) => {
    ...
    const [records, containers, folders, albumFolders] = await Promise.all([
      fetchAlbums(),
      fetchContainers(),
      fetchFolders(),
      fetchAlbumFolders()
  ...
  ipcMain.handle("import-xlsx", async (_event, payload = {}) => {
    ...
    const hasAnyFolderSheet = Boolean(containersWs || foldersWs || albumFoldersWs);
    ...
    if (!hasAnyFolderSheet) {
      ...
      [containers, folders, albumFolders] = await Promise.all([
        fetchContainers(),
        fetchFolders(),
        fetchAlbumFolders()
      ]);
      ...
      if (!foldersWs) {
        ...
        const [existingContainers, existingFolders, existingAlbumFolders] = await Promise.all([
          fetchContainers(),
          fetchFolders(),
          fetchAlbumFolders()
        ]);
