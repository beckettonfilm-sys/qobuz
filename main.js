...
function isJsonUpdateRecord(record) {
  const raw =
    record?.update ??
    record?.isUpdate ??
    record?.is_update ??
  ...
  if (typeof raw === "string") {
    return raw.trim().toLowerCase() === "update";
  }
  return false;
}
...
function buildMaxCoverUrl(url) {
  if (!url) return "";
  if (url.includes("320x320")) {
    return url.replace("320x320", "1280x1280");
  }
  return url;
}

function downloadBinaryFile(url, destination) {
...
async function ensureAlbumCovers({ appDirectory, albumId, pictureUrl }) {
  ...
  try {
    await downloadBinaryFile(pictureUrl, miniTarget);
  } catch (error) {
    usedDefault = true;
  ...
  try {
    ...
    await downloadBinaryFile(maxUrl, maxTarget);
  } catch (error) {
    usedDefault = true;
...
function registerHandlers() {
  ...
  ipcMain.handle("import-json", async (_event, payload = {}) => {
    const targetDir = payload?.directory || app.getAppPath() || __dirname;
    await ensureDirectory(targetDir);
    ...
    await ensureSchema();
    const { maxId, maxOrder, linkMap } = await getAlbumImportState();
    ...
    records.forEach((record) => {
      ...
      const isUpdate = isJsonUpdateRecord(record);
      ...
      const isDuplicate = Boolean(link && existingLinks.has(link));
      if (isDuplicate) {
        duplicates += 1;
      }
      ...
      if (isDuplicate || missingFields.length) {
        ...
        const albumId = isDuplicate ? linkMap.get(link) : nextId;
        if (albumId) {
      ...
      albumsToInsert.push({
        ...
        UPDATE_TS: isUpdate ? importStamp : null,
        row_order: nextOrder
      });
    ...
    for (const album of albumsToInsert) {
      const usedDefault = await ensureAlbumCovers({
        appDirectory,
        albumId: album.ID_ALBUMU,
        pictureUrl: album.PICTURE
      });
    ...
    if (duplicates) summaryLines.push(`🟡 Duplikaty (LINK): ${duplicates}`);
