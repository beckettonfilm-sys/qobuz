...
const ACTIVE_DB_META = path.join(BACKUP_DIR, "active_db.json");
...
async function readActiveDatabaseName() {
  if (!fs.existsSync(ACTIVE_DB_META)) return null;
  try {
    const payload = JSON.parse(await fs.promises.readFile(ACTIVE_DB_META, "utf8"));
    return payload?.fileName || null;
  } catch (error) {
    console.warn("⚠️ Nie udało się odczytać aktywnej bazy SQLite:", error);
    return null;
  }
}

async function writeActiveDatabaseName(fileName) {
  await ensureBackupDir();
  await fs.promises.writeFile(ACTIVE_DB_META, JSON.stringify({ fileName }, null, 2));
}
...
async function resolveDatabaseFile() {
  ...
  const activeName = await readActiveDatabaseName();
  if (activeName) {
    const activePath = path.join(BACKUP_DIR, activeName);
    if (fs.existsSync(activePath)) {
      dbFilePath = activePath;
      return dbFilePath;
    }
  }
  ...
  if (latest) {
    ...
    await writeActiveDatabaseName(latest.name);
    return dbFilePath;
  }

  const fileName = buildDatabaseFileName();
  dbFilePath = path.join(BACKUP_DIR, fileName);
  await writeActiveDatabaseName(fileName);
  return dbFilePath;
}
