...
function normalizeValue(column, value) {
  ...
  if (column === "duration" || column === "release_date" || column === "heard" || column === "id_albumu") {
...
async function replaceFolderData({ containers = [], folders = [], albumFolders = [] } = {}) {
  const db = await getDatabase();
  await run(db, "BEGIN TRANSACTION");
  try {
    await run(db, `DELETE FROM "${ALBUM_FOLDERS_TABLE_NAME}"`);
    ...
    if (containers.length) {
      ...
      const containerRows = containers.map((container, index) => {
        containerValues.push(container.name, Number(container.sort_order ?? index) || index);
        return "(?, ?)";
      });
      await run(
        db,
        `INSERT INTO "${CONTAINERS_TABLE_NAME}" (name, sort_order) VALUES ${containerRows.join(", ")}`,
        containerValues
      );
    }
