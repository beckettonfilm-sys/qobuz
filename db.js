...
async function createErrorContainer(db) {
...
async function importJsonAlbums({ records = [], errorAssignments = [] } = {}) {
  const db = await getDatabase();
  const result = {
    inserted: 0,
  ...
  try {
    ...
    if (errorAssignments.length) {
      const { name: containerName, id: containerId } = await createErrorContainer(db);
