...
async function migrateAlbumsTable(db, columnSet) {
  ...
  const artistExpr = getCol("artist", "NULL");
  const titleExpr = getCol("title", "NULL");
...
async function appendRecords(records = [], { markUpdated = false } = {}) {
  ...
  records.forEach((record) => {
    ...
    const rowValues = [];
    COLUMN_MAP.forEach(({ field, column }) => {
      const value = field === "UPDATE_TS" && updateStamp ? updateStamp : record[field];
      rowValues.push(normalizeValue(column, value));
    });
...
async function importJsonAlbums({ records = [], errorAssignments = [], collectionName } = {}) {
  const db = await getDatabase();
  const result = {
    inserted: 0,
  ...
  try {
    ...
    if (errorAssignments.length) {
      ...
      if (folderNameMap.size) {
        const maxSortRow = await all(
          db,
          `SELECT MAX(sort_order) as max_order FROM "${FOLDERS_TABLE_NAME}"`
        );
      ...
      const folderOrderMap = new Map(
        maxOrderRows.map((row) => [row.folder_id, Number(row.max_order) || 0])
      );
