...
def _ensure_storage():
    ...
    if not DATA_FILE.exists():
        ...
        ws.append([
            "SELECTOR","FOLDER","KONTENER","ADDED","LABEL","LINK",
            "PICTURE","ARTIST","TITLE","DURATION","RELEASE_DATE","Col_K","Col_F"
        ])
        wb.save(DATA_FILE)
...
def _update_excel(records, sheet_name):
    ...
    existing_headers = set(h for h in headers if h)
    ...
    header_lookup = { (h or '').upper(): h for h in headers if h }
    if "KONTENER" in header_lookup:
        header_lookup.setdefault("CONTAINER", header_lookup["KONTENER"])
    ...
    for rec in records:
        ...
        for key, value in rec_upper.items():
            ...
            if header and header in col_index:
                ws.cell(row=row, column=col_index[header]).value = value
        row += 1
...
if __name__ == "__main__":
    print(f"🚀 server_12.py startuje na http://127.0.0.1:5000")
    try:
