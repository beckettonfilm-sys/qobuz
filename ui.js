...
class UiController {
  ...
  async handleSaveFilterPreset() {
    ...
    if (this.uiState.filterPresets.length >= 30) {
      alert("Osiągnięto maksymalną ilość zapisanych filtrów.");
      return;
    }
    ...
    try {
      ...
      this.showTransientStatus(`${name.trim()} został zapisany ${formatStatusDate(new Date())}`);
    } catch (error) {
      alert(error.message || "Nie udało się zapisać filtrów.");
    }
  }
  ...
  async handlePresetRename() {
    ...
    try {
      ...
      this.showTransientStatus(
        `${nextName.trim()} zapisano edycję nazwy filtrów ${formatStatusDate(new Date())}`
      );
    } catch (error) {
