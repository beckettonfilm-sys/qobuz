...
class UiController {
  ...
  attachEvents() {
    ...
    downloadFilteredBtn?.addEventListener("click", () => this.exportFilteredSelection());
    ...
    exportFolderBtn?.addEventListener("click", () => {
      this.flashOptionButton(exportFolderBtn);
      this.exportFolderToXlsx();
    });

    importBtn?.addEventListener("click", () => {
      this.flashOptionButton(importBtn);
      this.importFromXlsx();
    });

    exportBtn?.addEventListener("click", () => {
      this.flashOptionButton(exportBtn);
      this.exportToXlsx();
    });

    modeSwitch?.addEventListener("click", () => {
      this.flashOptionButton(modeSwitch);
  ...
  buildOptionsPanel() {
    ...
    this.dom.exportFolderBtn = makeBtn("exportFolderBtn", "POBIERZ");
    this.dom.importBtn = makeBtn("importBtn", "IMPORT");
    this.dom.exportBtn = makeBtn("exportBtn", "EXPORT");
    this.dom.modeSwitch = makeBtn("modeSwitch", this.getManagementLabel());
  ...
  async exportToXlsx() {
    try {
      ...
      const response = await exportWorkbookToFile();
  ...
  toggleManagementMode() {
    ...
    if (modeSwitch) {
      modeSwitch.textContent = this.getManagementLabel();
  ...
  exportFilteredSelection() {
    ...
    const safeSheet = slugify(scopeLabel).slice(0, 25) || "wyniki";
    window.XLSX.utils.book_append_sheet(workbook, sheet, safeSheet);
    const filename = `filtered_${slugify(scopeLabel)}_sel-${slugify(selectorsLabel)}.xlsx`;
