...
class UiController {
  ...
  createAlbumCard(album) {
    ...
    const inDigitFolder = album.folder && album.folder !== "brak" && /^\d/.test(album.folder);
    dot.style.background = inDigitFolder ? "#4caf50" : "#fff";
  ...
  async handleSave() {
    ...
    try {
      ...
      alert(message);
      if (response.file_name || response.updated_at) {
        this.store.setFileMeta({
