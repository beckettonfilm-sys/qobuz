...
class UiController {
  ...
  updateDataDirectoryHint(operationKey) {
    ...
    const basePath = mode === "AUTO" ? this.getDefaultDirectory(operationKey) : state.manualDirectory;
    const label = basePath || "Wskaż folder docelowy";
    hint.textContent = `${mode} – ${label}`;
