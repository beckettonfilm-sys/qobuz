...
class UiController {
  ...
  openEditAlbumDialog(album) {
    ...
    return new Promise((resolve) => {
      ...
      const formatField = createLockableInput(this.resolveFormatLabel(album.format || ""), { locked: true });
      ...
      if (this.uiState.formatOptions.length) {
        const formatListId = `format-list-${album.id_albumu || Date.now()}`;
        const datalist = document.createElement("datalist");
        datalist.id = formatListId;
        this.uiState.formatOptions.forEach((entry) => {
          const option = document.createElement("option");
          option.value = entry.code;
          option.label = entry.full;
          option.textContent = entry.full;
          datalist.appendChild(option);
        });
        formatField.input.setAttribute("list", formatListId);
        form.appendChild(datalist);
      }
      const normalizeFormatInput = () => {
        const resolved = this.resolveFormatLabel(formatField.input.value);
        formatField.input.value = resolved;
      };
      formatField.input.addEventListener("change", normalizeFormatInput);
      formatField.input.addEventListener("blur", normalizeFormatInput);
      ...
      form.appendChild(buildRow("ROON ID", roonIdField));
      ...
      confirmBtn.addEventListener("click", async () => {
        ...
        const nextFormat = this.resolveFormatLabel(formatField.input.value.trim());
        const updates = {
