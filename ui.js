...
class UiController {
  constructor(store = new DataStore()) {
    ...
    this.uiState = {
      ...
      remixEnabled: false,
      remixSlots: Array.from({ length: 5 }, () => ({
        folder: "",
        percent: 100,
        mode: "percent",
        count: null
  ...
  buildFilterPanel() {
    ...
    clearBtn.textContent = "CLEAR FILTR";
  ...
  createRemixSection() {
    ...
    this.uiState.remixSlots.forEach((slot, index) => {
      ...
      this.attachHoldAction(percentLeft, () => this.shiftRemixValue(index, -1));
      this.attachHoldAction(percentRight, () => this.shiftRemixValue(index, 1));
      percentValue.addEventListener("keydown", (event) => {
        if (event.key !== "Enter") return;
        event.preventDefault();
        this.applyRemixValueInput(index, percentValue.value);
      });
      ...
      percentValue.addEventListener("contextmenu", (event) => {
        event.preventDefault();
        this.toggleRemixSlotMode(index);
      });
      ...
      unassignBtn.addEventListener("click", () => this.unassignRemixFolder(index));
      ...
      this.dom.remixSlots.push({ value, percentValue, percentValueWrap });
    });
    ...
    remixSection.appendChild(remixActions);
    this.refreshRemixSlotDisplays();
    this.updateRemixSearchSuggestions();
    return remixSection;
  }
  ...
  clearRemixSlots() {
    this.uiState.remixSlots = Array.from({ length: 5 }, () => ({
      folder: "",
      percent: 100,
      mode: "percent",
      count: null
  ...
  getRemixFolderForAlbum(album) {
    ...
    const remixFolders = this.uiState.remixSlots.map((slot) => slot.folder).filter(Boolean);
