...
class UiController {
  ...
  buildFilterPanel() {
    ...
    backupBtn.textContent = "Wykonaj backup DB";
    ...
    const tabs = [
      { id: "label", label: "WYTWÓRNIA", builder: () => this.createLabelsSection() },
      { id: "selector", label: "SELEKTOR", builder: () => this.createSelectorSection() },
      { id: "search", label: "WYSZUKIWARKA I DATA", builder: () => this.createSearchSection() },
      { id: "ops", label: "OPERACJE DANYCH", builder: () => this.createOperationsSection() },
      { id: "time", label: "CZAS", builder: () => this.createTimeSection() }
    ];
  ...
  createOperationsSection() {
    ...
    wrapper.appendChild(this.createSectionTitle("Operacje danych"));
    ...
    const switchConfigs = [
      ...
      { key: "download", label: "D_DB + DB_TXT" }
    ];
    ...
    optionsTitle.className = "filter-section__subtitle";
    optionsTitle.textContent = "Opcje";
    ...
    switchWrap.className = "option-switch";
    switchWrap.appendChild(modeSwitch.wrapper);
  ...
  createTimeSection() {
    ...
    wrapper.appendChild(this.createSectionTitle("Czas"));
  ...
  clearAllFilters() {
  const { filterPanel, releaseMonth, releaseYear, searchInput } = this.dom;
  ...
  if (releaseYear) releaseYear.value = "__all__";
  if (releaseMonth) releaseMonth.value = "__all__";
  ...
  renderPagination(totalPages, currentPage) {
    ...
    const createButton = ({ label, page, disabled = false }) => {
      ...
      btn.innerHTML = `<span class="menu-chip__inner">${label}</span>`;
      if (disabled) btn.disabled = true;
      return btn;
    };

    fragment.appendChild(
      createButton({ label: "POPRZEDNI", page: "prev", disabled: currentPage <= 0 || totalPages === 0 })
    );
  ...
  markFoldersPending() {
    ...
    if (this.uiState.foldersRefreshMode === "AUTO") {
  ...
  async handleDatabaseBackup() {
    ...
    try {
      ...
      this.startProgress("Tworzenie backupu bazy danych...");

      const response = await backupDatabase();
      const fileName = response?.backupFileName || "music_database.sqlite";

      this.finishProgress(`✅ Zapisano backup bazy danych: ${fileName}.`);
