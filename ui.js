...
class UiController {
  ...
  buildFilterPanel() {
    ...
    presetEditBtn.textContent = "EDIT";
    ...
    presetsWrap.appendChild(presetEditBtn);
  ...
  createTimeSection() {
    ...
    sortButtons.className = "filter-grid filter-time__sort";
    ...
    sortButtons.appendChild(ascChip.chip);
    sortButtons.appendChild(descChip.chip);

    sortButtons.appendChild(releaseDescChip.chip);
    sortButtons.appendChild(releaseAscChip.chip);
  ...
  updateFilterPresetOptions() {
    ...
    select.value = this.uiState.activeFilterPreset;
  }
  ...
  async handleSaveFilterPreset() {
    const name = window.prompt("Wpisz nazwę zestawu filtrów:");
    if (!name || !name.trim()) return;
    const payload = this.serializeCurrentFilters();
    try {
      await saveFilterPreset(name.trim(), payload);
      await this.loadFilterPresets();
      this.setActiveFilterPreset(name.trim());
    } catch (error) {
      alert(error.message || "Nie udało się zapisać filtrów.");
    }
  }
  ...
  applyFilterPreset(preset) {
    ...
    const parsedHeardMin = Number.isFinite(Number(payload.heardMin)) ? Number(payload.heardMin) : null;
    const parsedHeardMax = Number.isFinite(Number(payload.heardMax)) ? Number(payload.heardMax) : null;
    this.uiState.heardRange = {
      min: Number.isInteger(parsedHeardMin) ? parsedHeardMin : null,
      max: Number.isInteger(parsedHeardMax) ? parsedHeardMax : null
    };
    this.normalizeHeardRange();
    this.updateHeardRangeDisplay();

    const parsedDurationMin = Number(payload.durationMin);
    const parsedDurationMax = Number(payload.durationMax);
  ...
  async handlePresetRename() {
    ...
    const nextName = window.prompt("Podaj nową nazwę zestawu filtrów:", currentName);
    if (!nextName || !nextName.trim() || nextName.trim() === currentName) return;
    try {
      await renameFilterPreset(currentName, nextName.trim());
      await this.loadFilterPresets();
      this.setActiveFilterPreset(nextName.trim());
    } catch (error) {
      alert(error.message || "Nie udało się zmienić nazwy filtrów.");
    }
  }
