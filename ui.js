...
class UiController {
  constructor(store = new DataStore()) {
    ...
    this.uiState = {
      ...
      remixEnabled: false,
      remixSlots: Array.from({ length: 5 }, () => ({ folder: "", percent: 100 })),
      remixList: [],
      remixSearchTerm: "",
  ...
  cacheDom() {
    this.dom = {
      ...
      remixSearchHint: null,
      remixSlots: [],
      remixHeaderButton: null,
      remixHeaderDot: null,
      remixTabButton: null,
      remixTabDot: null,
      dataModeToggles: {},
  ...
  buildFilterPanel() {
    ...
    titleWrap.className = "filter-panel__title";

    const remixBar = document.createElement("div");
    remixBar.className = "filter-panel__remix-bar";
    const remixBtn = document.createElement("button");
    remixBtn.type = "button";
    remixBtn.className = "filter-tab__btn filter-panel__remix-btn";
    const remixLabel = document.createElement("span");
    remixLabel.className = "filter-tab__label";
    remixLabel.textContent = "REMIX";
    const remixDot = document.createElement("span");
    remixDot.className = "filter-tab__dot";
    remixLabel.appendChild(remixDot);
    remixBtn.appendChild(remixLabel);
    remixBtn.addEventListener("click", () => this.activateFilterTab?.("remix"));
    this.dom.remixHeaderButton = remixBtn;
    this.dom.remixHeaderDot = remixDot;
    ...
    remixBar.appendChild(remixBtn);
    remixBar.appendChild(remixToggleWrap);
    titleWrap.appendChild(remixBar);
  ...
  createRemixSection() {
    ...
    remixSection.appendChild(this.createSectionTitle("REMIX"));
    ...
    searchHint.className = "remix-search__hint";
    ...
    this.dom.remixSearchHint = searchHint;
    searchInput.addEventListener("input", () => {
      this.uiState.remixSearchTerm = searchInput.value;
      this.updateRemixSearchHint();
    });
    ...
    this.uiState.remixSlots.forEach((slot, index) => {
      ...
      percentValue.className = "remix-percent__value";
      ...
      percentLeft.addEventListener("click", () => this.shiftRemixPercent(index, -1));
      percentRight.addEventListener("click", () => this.shiftRemixPercent(index, 1));
      percentControl.appendChild(percentLeft);
      percentControl.appendChild(percentValue);
      percentControl.appendChild(percentRight);

      const assignBtn = document.createElement("button");
      assignBtn.type = "button";
      assignBtn.className = "remix-slot__assign";
      assignBtn.textContent = `PRZYPISZ FOLDER ${index + 1}`;
      assignBtn.addEventListener("click", () => this.assignRemixFolder(index));
      ...
      this.dom.remixSlots.push({ value, percentValue });
    });
  ...
  updateRemixSearchHint() {
    const hint = this.dom.remixSearchHint;
    if (!hint || !this.dom.remixSearchInput) return;
    const term = this.dom.remixSearchInput.value.trim();
    if (!term) {
      hint.textContent = "";
      return;
    }
    const matched = this.findRemixFolderMatch(term);
    if (!matched) {
      hint.textContent = "Nie znaleziono folderu";
      return;
    }
    const count = this.getRemixFolderCount(matched);
    hint.textContent = `${matched} (${count})`;
  }

  refreshRemixSlotDisplays() {
    if (!this.dom.remixSlots.length) return;
    this.uiState.remixSlots.forEach((slot, index) => {
      const slotDom = this.dom.remixSlots[index];
      if (!slotDom) return;
      const folderName = slot.folder;
      if (!folderName) {
        slotDom.value.textContent = "brak";
        slotDom.percentValue.textContent = `${slot.percent}%`;
        return;
      }
      const total = this.getRemixFolderCount(folderName);
      const applied = this.getRemixAppliedCount(total, slot.percent);
      slotDom.value.textContent = `${folderName} (${applied})`;
      slotDom.percentValue.textContent = `${slot.percent}%`;
    });
  }
  ...
  buildRemixList() {
    ...
    this.uiState.remixSlots.forEach((slot) => {
      if (!slot.folder) return;
      const allAlbums = this.store.getAlbumsForFolder(slot.folder);
      const total = allAlbums.length;
      if (!total) return;
      const targetCount = this.getRemixAppliedCount(total, slot.percent);
      const shuffled = this.shuffleArray(allAlbums);
      shuffled.slice(0, targetCount).forEach((album) => {
        if (album?.id_albumu) {
          unique.set(album.id_albumu, album);
        }
      });
    });
  ...
  updateFilterTabIndicators() {
    ...
    if (this.dom.remixHeaderDot) {
      this.dom.remixHeaderDot.classList.toggle("active", this.uiState.remixEnabled);
    }
  ...
  applyFilterPreset(preset) {
    ...
    this.uiState.remixSlots = Array.from({ length: 5 }, (_, index) => {
