...
class UiController {
  ...
  createTimeSection() {
    ...
    const heardActions = this.createActionsRow([
      {
        label: "CLEAR",
        handler: () => this.resetHeardFilterAndRender()
      }
    ]);
    ...
    const buildSortChip = ({ label, direction }) => {
      ...
      checkbox.checked = this.uiState.durationSort === direction;
      checkbox.addEventListener("change", () => {
        this.setDurationSort(direction);
      });
    ...
    const sortActions = this.createActionsRow([
      {
        label: "CLEAR",
        handler: () => this.resetDurationSortAndRender()
      }
    ]);
    ...
    const rangeActions = this.createActionsRow([
      {
        label: "CLEAR",
        handler: () => this.resetDurationRangeAndRender()
      }
    ]);

    wrapper.appendChild(heardRow);
    wrapper.appendChild(heardActions);
    wrapper.appendChild(sortTitle);
    wrapper.appendChild(sortButtons);
    wrapper.appendChild(sortActions);
    wrapper.appendChild(rangeTitle);
    wrapper.appendChild(rangeRow);
    wrapper.appendChild(rangeActions);
    return wrapper;
  }
  ...
  resetHeardFilterAndRender() {
    if (this.uiState.heardFilter === null) return;
  ...
  updateDurationSortButtons() {
    ...
    if (sortDurationAscBtn) {
      sortDurationAscBtn.checked = active === "asc";
    }
    if (sortDurationDescBtn) {
      sortDurationDescBtn.checked = active === "desc";
    }
  }

  resetDurationSortAndRender() {
    if (this.uiState.durationSort === null) return;
