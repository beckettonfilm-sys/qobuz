...
class UiController {
  ...
  openEditAlbumDialog(album) {
    ...
    return new Promise((resolve) => {
      ...
      const heading = document.createElement("h4");
      heading.className = "modal-title";
      heading.textContent = "EDYCJA_DANYCH";
      ...
      titleInput.className = "modal-input modal-input--row";
      titleInput.value = album.title || "";

      const artistInput = document.createElement("input");
      artistInput.type = "text";
      artistInput.className = "modal-input modal-input--row";
      ...
      releaseInput.className = "modal-input modal-input--row";
      releaseInput.value =
        album.release_original !== undefined && album.release_original !== null
          ? String(album.release_original)
          : album.release_date
      ...
      durationInput.className = "modal-input modal-input--row";
      durationInput.value = album.duration || 0;
      ...
      form.appendChild(buildRow("TITLE", titleInput));
      form.appendChild(buildRow("ARTIST", artistInput));
      form.appendChild(buildRow("LINK", linkInput));
      form.appendChild(buildRow("PICTURE", pictureInput));
      form.appendChild(buildRow("RELEASE_DATE", releaseInput));
      form.appendChild(buildRow("DURATION", durationInput));
      ...
      confirmBtn.addEventListener("click", () => {
        const releaseText = releaseInput.value.trim();
        const nextRelease = this.parseReleaseDateInput(releaseText);
        ...
        const updates = {
          ...
          release_original: releaseText || nextRelease || 0,
    ...
    try {
      ...
      const collectionName =
        this.uiState.activeCollection && this.uiState.activeCollection !== "__all__"
          ...
          : null;
      const response = await importJsonFromFile({ directory, collectionName });
      const summary = response?.summary || "✅ Import JSON zakończony.";
      const fileName = response?.fileName || "import.json";
