...
import { fetchWorkbook, updateWorkbook, logEvent } from "./api.js";
...
class UiController {
 ...
 processAndRender() {
  ...
  if (this.uiState.profilingEnabled) {
    logEvent({
      type: "processAndRender",
      durationMs: t1 - t0,
      category: this.uiState.currentCategory,
...
startOperation(message, opName = null) {
  const { fileStatus } = this.dom;
  this.uiState.operationInProgress = true;

  const name = opName || message;
  this.uiState.currentOperationName = name;
  this.uiState.currentOperationStartedAt = performance.now ? performance.now() : Date.now();
...
finishOperation(extra = {}) {
  const { fileStatus } = this.dom;
  this.uiState.operationInProgress = false;

  let durationMs = null;
  if (this.uiState.currentOperationStartedAt) {
    const now = performance.now ? performance.now() : Date.now();
    durationMs = now - this.uiState.currentOperationStartedAt;
  }

  if (this.uiState.profilingEnabled && this.uiState.currentOperationName) {
    logEvent({
      type: "operation-end",
      name: this.uiState.currentOperationName,
      durationMs,
      ...extra
    });
  }
  ...
  // reset
  this.uiState.currentOperationName = "";
  this.uiState.currentOperationStartedAt = 0;
  ...
  async handleCreateEntity() {
    ...
    logEvent({
      ...
      entityType: "folder",
      name,
      container: dialog.container || "brak"
    });
