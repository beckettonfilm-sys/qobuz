...
class DataStore {
  ...
  applyFilters({ releaseMonth = null, labels = null, searchTerm = "" } = {}) {
    const monthStartTs = releaseMonth ? Math.floor(new Date(releaseMonth.getFullYear(), releaseMonth.getMonth(), 1).getTime() / 1000) : null;
    const monthEndTs =
      releaseMonth
        ? Math.floor(new Date(releaseMonth.getFullYear(), releaseMonth.getMonth() + 1, 0, 23, 59, 59).getTime() / 1000)
        : null;
    ...
    if (changed) {
       this.activeFilters = { monthStartTs, monthEndTs, labelsKey, labelsSet, searchKey };
    }
  ...
  const { monthStartTs, monthEndTs, labelsSet, searchKey } = this.activeFilters;
  ...
  assignAlbumToFolder(album, targetFolder) {
    ...
    const folderInfo =
      normalizedTarget === "brak" ? { container: "brak" } : this.folderMeta.get(normalizedTarget) || this.ensureFolderEntry(normalizedTarget, "brak");
    const containerName = folderInfo.container || "brak";
    const targetStartsWithDigit = /^\d/.test(normalizedTarget);
    ...
    if (targetStartsWithDigit || normalizedTarget === "brak") {
      if (normalizedTarget === currentFolder && album.ory_copy !== "C") return null;
      const updates =
        normalizedTarget === "brak"
          ? { folder: "brak", container: "brak", added: "", added_ts: 0 }
          : { folder: normalizedTarget, container: containerName, added: addedStr, added_ts: now.getTime() };
