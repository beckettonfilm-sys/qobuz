...
class DataStore {
  ...
  applyFilters({
    ...
    const normalizedHeard = Number.isInteger(heardValue) ? clampHeard(heardValue) : null;
    ...
    const changed =
      ...
      this.activeFilters.heardValue !== normalizedHeard ||
  ...
  rebuildCategories(filters = null) {
    ...
    const { releaseStartTs, releaseEndTs, labelsSet, selectorsSet, searchKey, heardValue, durationMin, durationMax } =
      this.activeFilters;
    ...
    const passesFilters = (album) => {
      ...
      if (Number.isInteger(heardValue) && clampHeard(album.heard || 0) !== heardValue) return false;
  ...
  passesActiveFilters(albumLike) {
    ...
    const { releaseStartTs, releaseEndTs, labelsSet, selectorsSet, searchKey, durationMin, durationMax } =
      this.activeFilters;
    const allowedLabels = labelsSet || this.selectedLabels;
    const allowedSelectors = selectorsSet || this.selectedSelectors;
    if (!allowedLabels.has(album.label)) return false;
  ...
  removeAlbumFromCategories(album, prevSnapshot = null) {
    ...
    if (couldBeInNR) {
      this.removeFromList(this.categorized.NR, album);
    }
    this.newReleaseSet.delete(album);
  ...
  getCopyCounts() {
    const list = this.categorized.DB || [];
    return list.reduce(
      (acc, album) => {
        if (album.ory_copy === "C") acc.copies += 1;
        else acc.originals += 1;
        return acc;
      },
      { originals: 0, copies: 0 }
    );
  }
  ...
  getFilteredCategoryList(category, { folderFilter, containerFilter } = {}) {
    ...
    const sorted = [...list];
    if (this.durationSort) {
      const factor = this.durationSort === "asc" ? 1 : -1;
      sorted.sort((a, b) => {
        const left = Number(a.duration) || 0;
        const right = Number(b.duration) || 0;
        if (left === right) return (a.title || "").localeCompare(b.title || "", "pl");
  ...
  setDurationSort(direction) {
    ...
    if (this.durationSort === normalized) return false;
    this.durationSort = normalized;
    return true;
  }
